 ...
 * <td>boolean</td>
 * <td>true</td>
 * <td>Use Quasi-Newton (L-BFGS) optimization to find minimum. NOTE: Need to set
 * this to false if using other minimizers such as SGD.</td>
 * </tr>
 * <tr>
 * <td>QNsize</td>
 * <td>int</td>
 ...
 * features, after the first iteration - the first iteration is with
 * QNSize).</td>
 * </tr>
 * <tr>
 * <td>useInPlaceSGD</td>
 ...
 * <td>(null)</td>
 * <td>A different tokenizer factory to use if the ReaderAndWriter in question
 * uses tokenizers.</td>
 * </tr>
 * </table>
 *
...
public class SeqClassifierFlags implements Serializable {
	...
	// Turned on if non-null. Becomes part of the filename features are printed
	// to
	...
	public boolean largeChSegFile = false; // TODO this flag is used for
											// nothing;
	// delete when breaking serialization
	...
	public boolean useSpecialCharacters = false;
	public boolean usePhraseStructure = false;
	public boolean usePrevWithoutWord = false;
	public boolean useNextWithoutWord = false;
	public boolean useVowelsConsonants = false;

	// -----------------------------------------------------------------------

	/**
	 * Keep the whitespace between English words in testFile when printing out
	 * answers. Doesn't really change the content of the CoreLabels. (For
	...
	/**
	 * for Sighan bakeoff 2005, the path to the dictionary of bigrams appeared
	 * in corpus
	 */
	...
	public boolean printXML; // This is disused and can be removed when breaking
								// serialization
	...
	public boolean useNERPrior = false; // todo [cdm 2014]: Disused, to be
										// deleted, use priorModelFactory
	public boolean useAcqPrior = false; // todo [cdm 2014]: Disused, to be
										// deleted, use priorModelFactory

	public boolean useUniformPrior = false; // todo [cdm 2014]: Disused, to be
											// deleted, use priorModelFactory
	...
	public boolean useSemPrior = false; // todo [cdm 2014]: Disused, to be
										// deleted, use priorModelFactory
	...
	// This flag is only used for the sequences Type 2 CRF, not for
	// ie.crf.CRFClassifier
	...
	public int CRForder = 1; // TODO remove this when breaking serialization;
								// this is unused; really maxLeft/maxRight
								// control order
	public int CRFwindow = 2; // TODO remove this when breaking serialization;
								// this is unused; really maxLeft/maxRight
								// control clique size
	...
	// This is for use with the OWLQNMinimizer L1 regularization. To use it, set
	// useQN=false,
	// and this to a positive number. A smaller number means more features are
	// retained.
	...
	// for combining 2 CRFs - one trained from noisy data and another trained
	// from
	// non-noisy
	...
	// to know source of the label. Currently, used to know which pattern is
	// used
	// to label the token
	 ...
	 * distsim lexicon and for lookup. This is a simple word shaping that can
	 * shrink distsim lexicons and improve their performance.
	 */
	public boolean numberEquivalenceDistSim = false;

	/**
	 * What class to assign to words not found in the dist sim lexicon. You
	 * might want to make it a known class, if one is the "default class.
	 */
	public String unknownWordDistSimClass = "null";

	/**
	 * Use prefixes and suffixes from the previous and current word in edge
	 * clique.
	 */
	public boolean useNeighborNGrams = false;

	/**
	 * This function maps words in the training or test data to new words. They
	 * are used at the feature extractor level, ie in the FeatureFactory. For
	 * now, only the NERFeatureFactory uses this.
	 */
	...
	/**
	 * Use a bag of all words as a feature. Perhaps this will find some words
	 * that indicate certain types of entities are present.
	 */
	public boolean useBagOfWords = false;

	/**
	 * When scoring, count the background symbol stats too. Useful for things
	 * where the background symbol is particularly meaningful, such as truecase.
	 */
	public boolean evaluateBackground = false;

	/**
	 * Number of experts to be used in Logarithmic Opinion Pool (product of
	 * experts) training default value is 1
	 */
	...
	/**
	 * Whether or not to keep blank sentences when processing. Useful for
	 * systems such as the segmenter if you want to line up each line exactly,
	...
	public boolean useNERPriorBIO = false; // todo [cdm 2014]: Disused, to be
											// deleted, use priorModelFactory
	...
	// keenon: changed from = 1, nowadays it makes sense to default to
	// parallelism
	...
	public boolean useKnownLCWords = true; // disused, can be deleted when
	...
	/**
	 * A class name for a factory that vends a prior NER model that implements
	 * both SequenceModel and SequenceListener, and which is used in the Gibbs
	 * sampling sequence model inference.
	 */
	...
	public boolean splitSlashHyphenWords; // unused with new enum below. Remove
											// when breaking serialization.

	/**
	 * How many words it is okay to add to knownLCWords after initial training.
	 * If this number is negative, then add any number of further words during
	 * classifying/testing. If this number is non-negative (greater than or
	 * equal to 0), then add at most this many words to the knownLCWords. By
	 * default, this is now set to 0, so there is no transductive learning on
	 * the test set, since too many people complained about results changing
	 * over runs. However, traditionally we used a non-zero value, and this
	 * usually helps performance a bit (until 2014 it was -1, then it was set to
	 * 10_000, so that memory would not grow without bound if a
	 * SequenceClassifier is run for a long time.
	 */
	public int maxAdditionalKnownLCWords = 0; // was 10_000;

	public enum SlashHyphenEnum {
		NONE, WFRAG, WORD, BOTH
	};
	 ...
	 *            The properties object used for initialization
	 * @param printProps
	 *            Whether to print the properties on construction
	 */
	...
	public void setProperties(Properties props, boolean printProps) {
		...
		for (String key : props.stringPropertyNames()) {
			...
			} else if (key.equalsIgnoreCase("useSpecialCharacters")) {
				useSpecialCharacters = Boolean.parseBoolean(val);

			} else if (key.equalsIgnoreCase("usePeriod")) {
				usePeriod = Boolean.parseBoolean(val);
			} else if (key.equalsIgnoreCase("usePhraseStructure")) {
				usePhraseStructure = Boolean.parseBoolean(val);
			} else if (key.equalsIgnoreCase("usePrevWithoutWord")) {
				usePrevWithoutWord = Boolean.parseBoolean(val);
			} else if (key.equalsIgnoreCase("useNextWithoutWord")) {
				useNextWithoutWord = Boolean.parseBoolean(val);
			} else if (key.equalsIgnoreCase("useVowelsConsonants")) {
				useVowelsConsonants = Boolean.parseBoolean(val);
			}
			// -------------------------------------------------------

			else if (key.equalsIgnoreCase("goodCoNLL")) {
				// This was developed for CMMClassifier after the original 2003
				// CoNLL work.
				...
				if (Boolean.parseBoolean(val)) {
					...
					// noMidNGrams
					noMidNGrams = true;
					// should this be set?? maxNGramLeng = 6; No (to get best
					// score).
					// reverse
					useReverse = false;
					// typeseqs3
					...
					useNextRealWord = true;
					// smooth
					// This was originally 20, but in Aug 2006 increased to 50,
					// because that helped
					// for English, but actually even smaller than 20 helps for
					// languages like
					...
					strictGoodCoNLL = true; // don't add some CpC features added
											// later
					removeStrictGoodCoNLLDuplicates = true; // added in 2014;
															// the duplicated
															// features don't
															// help
					inputEncoding = "iso-8859-1"; // needed for CoNLL German and
													// Spanish files
					// optimization
					useQN = true;
					QNsize = 15;
				...
				if (Boolean.parseBoolean(val)) {
					...
					// this is slightly crude: it also turns off a few things
					// that
				...
				String[] tokens = val.split("\\s*,\\s*"); // multiple feature
															// factories could
															// be specified and
															// are comma
															// separated.
				...
				if (val.equalsIgnoreCase("linear") || val.equalsIgnoreCase("exp")
						|| val.equalsIgnoreCase("exponential")) {
					annealingType = val;
				} else {
				...
				// don't set if empty string or spaces or true: revert it to
				// null
				// special case so can empty out dictionary list on command
				// line!
				val = val.trim();
	...
	/**
	 * note that this does *not* return string representation of arrays, lists
	 * and enums
	 *
