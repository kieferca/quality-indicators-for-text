...
/**
 * Features for Named Entity Recognition. The code here creates the features by
 * processing Lists of CoreLabels. Look at {@link SeqClassifierFlags} to see
 * where the flags are set for what options to use for what flags.
 * <p>
 * To add a new feature extractor, you should do the following:
 * <ol>
 * <li>Add a variable (boolean, int, String, etc. as appropriate) to
 * SeqClassifierFlags to mark if the new extractor is turned on or its value,
 * etc. Add it at the <i>bottom</i> of the list of variables currently in the
 * class (this avoids problems with older serialized files breaking). Make the
 * default value of the variable false/null/0 (this is again for backwards
 * compatibility).</li>
 * <li>Add a clause to the big if/then/else of setProperties(Properties) in
 * SeqClassifierFlags. Unless it is a macro option, make the option name the
 * same as the variable name used in step 1.</li>
 ...
 * class, you add the feature extractor to the {@code featuresC} code, if both
 * the current and previous class, then {@code featuresCpC}, etc.</li>
 * </ol>
 * <p>
 ...
 * line. The following properties are recognized:
 * </p>
 * <table border="1">
 * <tr>
 ...
 * </tr>
 * <tr>
 * <td>loadClassifier</td>
 * <td>String</td>
 * <td>n/a</td>
 * <td>Path to serialized classifier to load</td>
 * </tr>
 * <tr>
 * <td>loadAuxClassifier</td>
 * <td>String</td>
 * <td>n/a</td>
 * <td>Path to auxiliary classifier to load.</td>
 * </tr>
 * <tr>
 * <td>serializeTo</td>
 * <td>String</td>
 * <td>n/a</td>
 * <td>Path to serialize classifier to</td>
 * </tr>
 * <tr>
 * <td>trainFile</td>
 * <td>String</td>
 * <td>n/a</td>
 * <td>Path of file to use as training data</td>
 * </tr>
 * <tr>
 * <td>testFile</td>
 * <td>String</td>
 ...
 * <td>String</td>
 * <td>see below</td>
 * <td>This applies at training time or if testing on tab-separated column data.
 * It says what is in each column. It doesn't apply when running on plain text
 * data. The simplest scenario for training is having words and classes in two
 * column. word=0,answer=1 is the default if conllNoTags is specified; otherwise
 * word=0,tag=1,answer=2 is the default. But you can add other columns, such as
 * for a part-of-speech tag, presences in a lexicon, etc. That would only be
 * useful at runtime if you have part-of-speech information or whatever
 * available and are passing it in with the tokens (that is, you can pass to
 * classify CoreLabel tokens with additional fields stored in them).</td>
 * </tr>
 * <tr>
 * <td>useWord</td>
 * <td>boolean</td>
 * <td>true</td>
 * <td>Gives you feature for w</td>
 * </tr>
 * <tr>
 * <td>useBinnedLength</td>
 * <td>String</td>
 * <td>null</td>
 * <td>If non-null, treat as a sequence of comma separated integer bounds, where
 * items above the previous bound up to the next bound are binned
 * Len-<i>range</i></td>
 * </tr>
 * <tr>
 * <td>useNGrams</td>
 * <td>boolean</td>
 * <td>false</td>
 * <td>Make features from letter n-grams, i.e., substrings of the word</td>
 * </tr>
 * <tr>
 * <td>lowercaseNGrams</td>
 * <td>boolean</td>
 * <td>false</td>
 * <td>Make features from letter n-grams only lowercase</td>
 * </tr>
 * <tr>
 * <td>dehyphenateNGrams</td>
 * <td>boolean</td>
 * <td>false</td>
 * <td>Remove hyphens before making features from letter n-grams</td>
 * </tr>
 * <tr>
 * <td>conjoinShapeNGrams</td>
 * <td>boolean</td>
 * <td>false</td>
 * <td>Conjoin word shape and n-gram features</td>
 * </tr>
 * <tr>
 * <td>useNeighborNGrams</td>
 * <td>boolean</td>
 * <td>false</td>
 * <td>Use letter n-grams for the previous and current words in the CpC clique.
 * This feature helps languages such as Chinese, but not so much for
 * English</td>
 * </tr>
 * <tr>
 * <td>useMoreNeighborNGrams</td>
 * <td>boolean</td>
 * <td>false</td>
 * <td>Use letter n-grams for the previous and next words in the C clique. This
 * feature helps languages such as Chinese, but not so much for English</td>
 * </tr>
 * <tr>
 * <td>usePrev</td>
 * <td>boolean</td>
 * <td>false</td>
 * <td>Gives you feature for (pw,c), and together with other options enables
 * other previous features, such as (pt,c) [with useTags)</td>
 * </tr>
 * <tr>
 * <td>useNext</td>
 * <td>boolean</td>
 * <td>false</td>
 * <td>Gives you feature for (nw,c), and together with other options enables
 * other next features, such as (nt,c) [with useTags)</td>
 * </tr>
 * <tr>
 * <td>useTags</td>
 * <td>boolean</td>
 * <td>false</td>
 * <td>Gives you features for (t,c), (pt,c) [if usePrev], (nt,c) [if
 * useNext]</td>
 * </tr>
 * <tr>
 * <td>useWordPairs</td>
 * <td>boolean</td>
 * <td>false</td>
 * <td>Gives you features for (pw, w, c) and (w, nw, c)</td>
 * </tr>
 *
 * <tr>
 * <td>useGazettes</td>
 * <td>boolean</td>
 * <td>false</td>
 * <td>If true, use gazette features (defined by other flags)</td>
 * </tr>
 * <tr>
 * <td>gazette</td>
 * <td>String</td>
 * <td>null</td>
 * <td>The value can be one or more filenames (names separated by a comma,
 * semicolon or space). If provided gazettes are loaded from these files. Each
 * line should be an entity class name, followed by whitespace followed by an
 * entity (which might be a phrase of several tokens with a single space between
 * words). Giving this property turns on useGazettes, so you normally don't need
 * to specify it (but can use it to turn off gazettes specified in a properties
 * file).</td>
 * </tr>
 * <tr>
 * <td>sloppyGazette</td>
 * <td>boolean</td>
 * <td>false</td>
 * <td>If true, a gazette feature fires when any token of a gazette entry
 * matches</td>
 * </tr>
 * <tr>
 * <td>cleanGazette</td>
 * <td>boolean</td>
 * <td>false</td>
 * <td>If true, a gazette feature fires when all tokens of a gazette entry
 * match</td>
 * </tr>
 *
 * <tr>
 ...
 * <td>String</td>
 * <td>none</td>
 * <td>Either "none" for no wordShape use, or the name of a word shape function
 * recognized by {@link WordShapeClassifier#lookupShaper(String)}</td>
 * </tr>
 * <tr>
 * <td>useSequences</td>
 * <td>boolean</td>
 * <td>true</td>
 * <td>Does not use any class combination features if this is false</td>
 * </tr>
 * <tr>
 * <td>usePrevSequences</td>
 * <td>boolean</td>
 * <td>false</td>
 * <td>Does not use any class combination features using previous classes if
 * this is false</td>
 * </tr>
 * <tr>
 * <td>useNextSequences</td>
 * <td>boolean</td>
 * <td>false</td>
 * <td>Does not use any class combination features using next classes if this is
 * false</td>
 * </tr>
 * <tr>
 * <td>useLongSequences</td>
 * <td>boolean</td>
 * <td>false</td>
 * <td>Use plain higher-order state sequences out to minimum of length or
 * maxLeft</td>
 * </tr>
 * <tr>
 * <td>useBoundarySequences</td>
 * <td>boolean</td>
 * <td>false</td>
 * <td>Use extra second order class sequence features when previous is CoNLL
 * boundary, so entity knows it can span boundary.</td>
 * </tr>
 * <tr>
 * <td>useTaggySequences</td>
 * <td>boolean</td>
 * <td>false</td>
 * <td>Use first, second, and third order class and tag sequence interaction
 * features</td>
 * </tr>
 * <tr>
 * <td>useExtraTaggySequences</td>
 * <td>boolean</td>
 * <td>false</td>
 * <td>Add in sequences of tags with just current class features</td>
 * </tr>
 * <tr>
 * <td>useTaggySequencesShapeInteraction</td>
 * <td>boolean</td>
 * <td>false</td>
 * <td>Add in terms that join sequences of 2 or 3 tags with the current
 * shape</td>
 * </tr>
 * <tr>
 * <td>strictlyFirstOrder</td>
 * <td>boolean</td>
 * <td>false</td>
 * <td>As an override to whatever other options are in effect, deletes all
 * features other than C and CpC clique features when building the
 * classifier</td>
 * </tr>
 * <tr>
 * <td>entitySubclassification</td>
 * <td>String</td>
 * <td>"IO"</td>
 * <td>If set, convert the labeling of classes (but not the background) into one
 * of several alternate encodings (IO, IOB1, IOB2, IOE1, IOE2, SBIEO, with a
 * S(ingle), B(eginning), E(nding), I(nside) 4-way classification for each
 * class. By default, we either do no re-encoding, or the
 * CoNLLDocumentIteratorFactory does a lossy encoding as IO. Note that this is
 * all CoNLL-specific, and depends on their way of prefix encoding classes, and
 * is only implemented by the CoNLLDocumentIteratorFactory.</td>
 * </tr>
 *
 * <tr>
 * <td>useSum</td>
 ...
 * <td>double</td>
 * <td>1e-4</td>
 * <td>Convergence tolerance in optimization</td>
 * </tr>
 * <tr>
 * <td>printFeatures</td>
 * <td>String</td>
 * <td>null</td>
 * <td>print out all the features generated by the classifier for a dataset to a
 * file based on this name (starting with "features-", suffixed "-1" and "-2"
 * for train and test). This simply prints the feature names, one per line.</td>
 * </tr>
 * <tr>
 * <td>printFeaturesUpto</td>
 * <td>int</td>
 * <td>-1</td>
 * <td>Print out features for only the first this many datums, if the value is
 * positive.</td>
 * </tr>
 *
 * <tr>
 ...
 * <td>boolean</td>
 * <td>false</td>
 * <td>Gives you features (pt, t, nt, c), (t, nt, c), (pt, t, c)</td>
 * </tr>
 * <tr>
 * <td>useSymWordPairs</td>
 * <td>boolean</td>
 * <td>false</td>
 * <td>Gives you features (pw, nw, c)</td>
 * </tr>
 *
 * <tr>
 * <td>printClassifier</td>
 * <td>String</td>
 * <td>null</td>
 * <td>Style in which to print the classifier. One of: HighWeight,
 * HighMagnitude, Collection, AllWeights, WeightHistogram</td>
 * </tr>
 * <tr>
 * <td>printClassifierParam</td>
 * <td>int</td>
 * <td>100</td>
 * <td>A parameter to the printing style, which may give, for example the number
 * of parameters to print</td>
 * </tr>
 * <tr>
 * <td>intern</td>
 * <td>boolean</td>
 * <td>false</td>
 * <td>If true, (String) intern read in data and classes and feature (pre-)names
 * such as substring features</td>
 * </tr>
 * <tr>
 * <td>intern2</td>
 * <td>boolean</td>
 * <td>false</td>
 * <td>If true, intern all (final) feature names (if only current word and ngram
 * features are used, these will already have been interned by intern, and this
 * is an unnecessary no-op)</td>
 * </tr>
 * <tr>
 * <td>cacheNGrams</td>
 * <td>boolean</td>
 * <td>false</td>
 * <td>If true, record the NGram features that correspond to a String (under the
 * current option settings) and reuse rather than recalculating if the String is
 * seen again.</td>
 * </tr>
 * <tr>
 * <td>selfTest</td>
 ...
 * <td>boolean</td>
 * <td>false</td>
 * <td>Do not include character n-gram features for n-grams that contain neither
 * the beginning or end of the word</td>
 * </tr>
 * <tr>
 * <td>maxNGramLeng</td>
 * <td>int</td>
 * <td>-1</td>
 * <td>If this number is positive, n-grams above this size will not be used in
 * the model</td>
 * </tr>
 * <tr>
 * <td>useReverse</td>
 ...
 * </tr>
 * <tr>
 * <td>retainEntitySubclassification</td>
 * <td>boolean</td>
 * <td>false</td>
 * <td>If true, rather than undoing a recoding of entity tag subtypes (such as
 * BIO variants), just leave them in the output.</td>
 * </tr>
 * <tr>
 * <td>useLemmas</td>
 * <td>boolean</td>
 * <td>false</td>
 * <td>Include the lemma of a word as a feature.</td>
 * </tr>
 * <tr>
 * <td>usePrevNextLemmas</td>
 * <td>boolean</td>
 * <td>false</td>
 * <td>Include the previous/next lemma of a word as a feature.</td>
 * </tr>
 * <tr>
 * <td>useLemmaAsWord</td>
 * <td>boolean</td>
 * <td>false</td>
 * <td>Include the lemma of a word as a feature.</td>
 * </tr>
 * <tr>
 * <td>normalizeTerms</td>
 * <td>boolean</td>
 * <td>false</td>
 * <td>If this is true, some words are normalized: day and month names are
 * lowercased (as for normalizeTimex) and some British spellings are mapped to
 * American English spellings (e.g., -our/-or, etc.).</td>
 * </tr>
 * <tr>
 * <td>normalizeTimex</td>
 * <td>boolean</td>
 * <td>false</td>
 * <td>If this is true, capitalization of day and month names is normalized to
 * lowercase</td>
 * </tr>
 * <tr>
 * <td>useNB</td>
 ...
 * <td>boolean</td>
 * <td>false</td>
 * <td>Use basic zeroeth order word shape features.</td>
 * </tr>
 * <tr>
 * <td>useTypeSeqs2</td>
 * <td>boolean</td>
 * <td>false</td>
 * <td>Add additional first and second order word shape features</td>
 * </tr>
 * <tr>
 * <td>useTypeSeqs3</td>
 * <td>boolean</td>
 * <td>false</td>
 * <td>Adds one more first order shape sequence</td>
 * </tr>
 * <tr>
 * <td>useDisjunctive</td>
 * <td>boolean</td>
 * <td>false</td>
 * <td>Include in features giving disjunctions of words anywhere in the left or
 * right disjunctionWidth words (preserving direction but not position)</td>
 * </tr>
 * <tr>
 * <td>disjunctionWidth</td>
 * <td>int</td>
 * <td>4</td>
 * <td>The number of words on each side of the current word that are included in
 * the disjunction features</td>
 * </tr>
 * <tr>
 * <td>useDisjunctiveShapeInteraction</td>
 * <td>boolean</td>
 * <td>false</td>
 * <td>Include in features giving disjunctions of words anywhere in the left or
 * right disjunctionWidth words (preserving direction but not position)
 * interacting with the word shape of the current word</td>
 * </tr>
 * <tr>
 * <td>useWideDisjunctive</td>
 * <td>boolean</td>
 * <td>false</td>
 * <td>Include in features giving disjunctions of words anywhere in the left or
 * right wideDisjunctionWidth words (preserving direction but not position)</td>
 * </tr>
 * <tr>
 * <td>wideDisjunctionWidth</td>
 * <td>int</td>
 * <td>4</td>
 * <td>The number of words on each side of the current word that are included in
 * the disjunction features</td>
 ...
 * <td>boolean</td>
 * <td>false</td>
 * <td>Use combination of position in sentence and class as a feature</td>
 * </tr>
 * <tr>
 * <td>useBeginSent</td>
 * <td>boolean</td>
 * <td>false</td>
 * <td>Use combination of initial position in sentence and class (and word
 * shape) as a feature. (Doesn't seem to help.)</td>
 * </tr>
 * <tr>
 * <td>useDisjShape</td>
 * <td>boolean</td>
 * <td>false</td>
 * <td>Include features giving disjunctions of word shapes anywhere in the left
 * or right disjunctionWidth words (preserving direction but not position)</td>
 * </tr>
 * <tr>
 * <td>useClassFeature</td>
 * <td>boolean</td>
 * <td>false</td>
 * <td>Include a feature for the class (as a class marginal). Puts a prior on
 * the classes which is equivalent to how often the feature appeared in the
 * training data. This is the same thing as having a bias vector or having an
 * always-on feature in a model.</td>
 * </tr>
 * <tr>
 * <td>useShapeConjunctions</td>
 * <td>boolean</td>
 * <td>false</td>
 * <td>Conjoin shape with tag or position</td>
 * </tr>
 * <tr>
 * <td>useWordTag</td>
 * <td>boolean</td>
 * <td>false</td>
 * <td>Include word and tag pair features</td>
 * </tr>
 * <tr>
 * <td>useLastRealWord</td>
 * <td>boolean</td>
 * <td>false</td>
 * <td>Iff the prev word is of length 3 or less, add an extra feature that
 * combines the word two back and the current word's shape. <i>Weird!</i></td>
 * </tr>
 * <tr>
 * <td>useNextRealWord</td>
 * <td>boolean</td>
 * <td>false</td>
 * <td>Iff the next word is of length 3 or less, add an extra feature that
 * combines the word after next and the current word's shape. <i>Weird!</i></td>
 * </tr>
 * <tr>
 * <td>useTitle</td>
 * <td>boolean</td>
 * <td>false</td>
 * <td>Match a word against a list of name titles (Mr, Mrs, etc.). Doesn't
 * really seem to help.</td>
 * </tr>
 * <tr>
 * <td>useTitle2</td>
 * <td>boolean</td>
 * <td>false</td>
 * <td>Match a word against a better list of English name titles (Mr, Mrs,
 * etc.). Still doesn't really seem to help.</td>
 * </tr>
 * <tr>
 * <td>useDistSim</td>
 * <td>boolean</td>
 * <td>false</td>
 * <td>Load a file of distributional similarity classes (specified by
 * {@code distSimLexicon}) and use it for features</td>
 * </tr>
 * <tr>
 * <td>distSimLexicon</td>
 * <td>String</td>
 * <td></td>
 * <td>The file to be loaded for distsim classes.</td>
 * </tr>
 * <tr>
 * <td>distSimFileFormat</td>
 * <td>String</td>
 * <td>alexclark</td>
 * <td>Files should be formatted as tab separated rows where each row is a
 * word/class pair. alexclark=word first, terrykoo=class first</td>
 * </tr>
 * <tr>
 * <td>useOccurrencePatterns</td>
 * <td>boolean</td>
 * <td>false</td>
 * <td>This is a very engineered feature designed to capture multiple references
 * to names. If the current word isn't capitalized, followed by a
 * non-capitalized word, and preceded by a word with alphabetic characters, it
 * returns NO-OCCURRENCE-PATTERN. Otherwise, if the previous word is a
 * capitalized NNP, then if in the next 150 words you find this PW-W sequence,
 * you get XY-NEXT-OCCURRENCE-XY, else if you find W you get
 * XY-NEXT-OCCURRENCE-Y. Similarly for backwards and XY-PREV-OCCURRENCE-XY and
 * XY-PREV-OCCURRENCE-Y. Else (if the previous word isn't a capitalized NNP),
 * under analogous rules you get one or more of X-NEXT-OCCURRENCE-YX,
 * X-NEXT-OCCURRENCE-XY, X-NEXT-OCCURRENCE-X, X-PREV-OCCURRENCE-YX,
 * X-PREV-OCCURRENCE-XY, X-PREV-OCCURRENCE-X.</td>
 * </tr>
 * <tr>
 * <td>useTypeySequences</td>
 * <td>boolean</td>
 * <td>false</td>
 * <td>Some first order word shape patterns.</td>
 * </tr>
 * <tr>
 * <td>useGenericFeatures</td>
 * <td>boolean</td>
 * <td>false</td>
 * <td>If true, any features you include in the map will be incorporated into
 * the model with values equal to those given in the file; values are treated as
 * strings unless you use the "realValued" option (described below)</td>
 * </tr>
 * <tr>
 * <td>justify</td>
 * <td>boolean</td>
 * <td>false</td>
 * <td>Print out all feature/class pairs and their weight, and then for each
 * input data point, print justification (weights) for active features. Only
 * implemented for CMMClassifier.</td>
 * </tr>
 * <tr>
 * <td>normalize</td>
 * <td>boolean</td>
 * <td>false</td>
 * <td>For the CMMClassifier (only) if this is true then the Scorer normalizes
 * scores as probabilities.</td>
 * </tr>
 * <tr>
 * <td>useHuber</td>
 * <td>boolean</td>
 * <td>false</td>
 * <td>Use a Huber loss prior rather than the default quadratic loss.</td>
 * </tr>
 * <tr>
 * <td>useQuartic</td>
 * <td>boolean</td>
 * <td>false</td>
 * <td>Use a Quartic prior rather than the default quadratic loss.</td>
 * </tr>
 * <tr>
 * <td>sigma</td>
 * <td>double</td>
 ...
 * <td>double</td>
 * <td>0.01</td>
 * <td>Used only as a parameter in the Huber loss: this is the distance from 0
 * at which the loss changes from quadratic to linear</td>
 * </tr>
 * <tr>
 * <td>beamSize</td>
 * <td>int</td>
 ...
 * <td>int</td>
 * <td>2</td>
 * <td>The number of things to the left that have to be cached to run the
 * Viterbi algorithm: the maximum context of class features used.</td>
 * </tr>
 * <tr>
 * <td>maxRight</td>
 * <td>int</td>
 * <td>2</td>
 * <td>The number of things to the right that have to be cached to run the
 * Viterbi algorithm: the maximum context of class features used. The maximum
 * possible clique size to use is (maxLeft + maxRight + 1)</td>
 * </tr>
 * <tr>
 * <td>dontExtendTaggy</td>
 * <td>boolean</td>
 * <td>false</td>
 * <td>Don't extend the range of useTaggySequences when maxLeft is
 * increased.</td>
 * </tr>
 * <tr>
 * <td>numFolds</td>
 * <td>int</td>
 * <td>1</td>
 * <td>The number of folds to use for cross-validation. CURRENTLY NOT
 * IMPLEMENTED.</td>
 * </tr>
 * <tr>
 * <td>startFold</td>
 * <td>int</td>
 * <td>1</td>
 * <td>The starting fold to run. CURRENTLY NOT IMPLEMENTED.</td>
 * </tr>
 * <tr>
 * <td>endFold</td>
 * <td>int</td>
 * <td>1</td>
 * <td>The last fold to run. CURRENTLY NOT IMPLEMENTED.</td>
 * </tr>
 * <tr>
 * <td>mergeTags</td>
 * <td>boolean</td>
 * <td>false</td>
 * <td>Whether to merge B- and I- tags.</td>
 * </tr>
 * <tr>
 * <td>splitDocuments</td>
 * <td>boolean</td>
 * <td>true</td>
 * <td>Whether or not to split the data into separate documents for
 * training/testing</td>
 * </tr>
 * <tr>
 * <td>maxDocSize</td>
 * <td>int</td>
 * <td>10000</td>
 * <td>If this number is greater than 0, attempt to split documents bigger than
 * this value into multiple documents at sentence boundaries during testing;
...
public class NERFeatureFactory<IN extends CoreLabel> extends FeatureFactory<IN> {
	 ...
	 * @param loc
	 *            The index at which to extract features.
	 */
	@Override
	public Collection<String> getCliqueFeatures(PaddedList<IN> cInfo, int loc, Clique clique) {
		...
		String suffix;
		if (clique == cliqueC) {
			// 200710: tried making this clique null; didn't improve performance
			// (rafferty)
	...
	private static final Pattern ordinalPattern = Pattern.compile("(?:(?:first|second|third|fourth|fifth|"
			...
			+ "seventeenth|eighteenth|nineteenth|" + "twenty|twentieth|thirty|thirtieth|"
			+ "forty|fortieth|fifty|fiftieth|" + "sixty|sixtieth|seventy|seventieth|"
			+ "eighty|eightieth|ninety|ninetieth|" + "one|two|three|four|five|six|seven|"
	...
	/**
	 * Reads a gazette file. Each line of it consists of a class name (a String
	 * not containing whitespace characters), followed by whitespace characters
	 * followed by a phrase, which is one or more tokens separated by a single
	 * space.
	 *
	 * @param in
	 *            Where to read the gazette from
	 * @throws IOException
	 *             If IO errors
	 */
	...
	private Set<Class<? extends GenericAnnotation<?>>> genericAnnotationKeys; // =
																				// null;
																				// //cache
																				// which
																				// keys
	...
	private final Pattern titlePattern = Pattern.compile("(?:Mr|Ms|Mrs|Dr|Miss|Sen|Judge|Sir)\\.?"); // todo:
																										// should
																										// make
																										// static
																										// final
																										...
																										// more
																										// titles
	private static final Pattern titlePattern2 = Pattern.compile(
	...
	private void generateSlashHyphenFeatures(String word, Collection<String> featuresC, String fragSuffix,
			String wordSuffix) {
	...
	protected Collection<String> featuresC(PaddedList<IN> cInfo, int loc) {
		...
		if (flags.useLength) {
			...
			sb.append(cWord.length());
			sb.append("-Length");
			featuresC.add(sb.toString());
		}

		// whether the word contains special characters
		if (flags.useSpecialCharacters) {
			Pattern pattern = Pattern.compile("[/\\$&+*,:;=?#|-]");
			Matcher m = pattern.matcher(cWord);
			if (m.find()) {
				StringBuilder sb = new StringBuilder();
				sb.append("TRUE");
				sb.append("-HasSpecialCharacters");
				featuresC.add(sb.toString());
			}
		}
		
		// whether a period is contained in the word
		if (flags.usePeriod) {
			Pattern pattern = Pattern.compile("[.]");
			Matcher m = pattern.matcher(cWord);
			if (m.find()) {
		...
		// feature for dependency parsed annotation
		if (flags.usePhraseStructure) {
			featuresC.add(c.getString(CoreAnnotations.PhraseStructureAnnotation.class) + "-PHRASESTRUCTURE");
		}


		/*
		 * 2.  Features derived from the combination of vowel letters and consonant letters, which include:  
		 * a) whether all letters are vowels; 
		 * b) whether all letters are consonants; c) whether both vowel letters and consonant letters are present. 
		 */
		if (flags.useVowelsConsonants) {
			char[] vowels = {'a','ä','e','i','o','ö','u','ü'};
			char[] punctuations = {'!','"','#','$','%','&','\'','(',')','*','+',',','-','.','/',':',';','<','=','>','?','@','[','\\',']','^','_','`','{','|','}','~'};
		    char[] whitespaces = {' ','\t','\n','\f','\r'};
			StringBuilder sb = new StringBuilder("VCWS-");
		    int len = cWord.length();
		    for (int i = 0; i < len; i++) {
		      char character = cWord.charAt(i);
		      char m = character;
		      if (Character.isDigit(character)) {
		        ...
		        for(char vowel : vowels) {
			    	...
			    	m = 'C';
			        for(char vowel : vowels) {
			        	if (vowel == Character.toLowerCase(character)){
			        		m = 'V';
			        	}
			        }
		      } else {
		...
		// own feature for previous token but without adding the actual previous word as feature
		if (flags.usePrevWithoutWord) {
			//featuresC.add(pWord + "-PW");
			if (flags.useTags) {
				featuresC.add(p.getString(CoreAnnotations.PartOfSpeechAnnotation.class) + "-PTAG");
			}
			if (flags.usePhraseStructure) {
				featuresC.add(p.getString(CoreAnnotations.PhraseStructureAnnotation.class) + "-PPHRASESTRUCTURE");
			}
			if (flags.useDistSim) {
		...
		// own feature for next token but without adding the actual next word as feature
		if (flags.useNextWithoutWord) {
			//featuresC.add(nWord + "-NW");
			if (flags.useTags) {
				featuresC.add(n.getString(CoreAnnotations.PartOfSpeechAnnotation.class) + "-NTAG");
			}
			if (flags.usePhraseStructure) {
				featuresC.add(n.getString(CoreAnnotations.PhraseStructureAnnotation.class) + "-NPHRASESTRUCTURE");
			}
			if (flags.useDistSim) {
		...
		if (flags.useInternal && flags.useExternal) {
			...
			String pGazAnnotation = (flags.useGazFeatures || flags.useMoreGazFeatures)
					? p.get(CoreAnnotations.GazAnnotation.class) : null;
			String nGazAnnotation = (flags.useGazFeatures || flags.useMoreGazFeatures)
					? n.get(CoreAnnotations.GazAnnotation.class) : null;
			String cGazAnnotation = (flags.useGazFeatures || flags.useMoreGazFeatures)
					? c.get(CoreAnnotations.GazAnnotation.class) : null;
			...
			if (flags.useAbbr) {
				...
				featuresC.add(
						p.get(CoreAnnotations.AbbrAnnotation.class) + '-' + c.get(CoreAnnotations.AbbrAnnotation.class)
			...
			if (flags.useNPHead) {
				...
				if (flags.useTags) {
					featuresC.add(c.get(CoreAnnotations.HeadWordStringAnnotation.class) + "-"
			...
			if (flags.useNGrams) {
				...
				if (subs == null) {
					...
					// hoist flags.noMidNGrams so only linear in word length for
					// that case
			...
			if (flags.useLastRealWord) {
				if (pWord.length() <= 3) {
					// extending this to check for 2 short words doesn't seem to
					// help....
			...
			if (flags.useDisjShape) {
				for (int i = 1; i <= flags.disjunctionWidth; i++) {
					...
					// featuresC.add(cInfo.get(loc -
					// i).get(CoreAnnotations.ShapeAnnotation.class) +
					// "-PDISJSHAPE");
			...
			if ((flags.wordShape > WordShapeClassifier.NOWORDSHAPE) || (flags.useShapeStrings)) {
				if (flags.useTypeSeqs) {
					...
					featuresC.add(cShape + "..." + nWord + "-NW_CTYPE");
					if (flags.maxLeft > 0)
						featuresC.add(pShape + "..." + cShape + "-PCTYPE"); // this
																			// one
																			// just
																			// isn't
																			// useful,
		...
		if (flags.useGenericFeatures) {
			...
			for (Class key : genericAnnotationKeys) {
				...
				// CoreLabel.genericValues.get(key) + " with value " +
				// c.get(key));
		...
		if (flags.useTopics) {
			// featuresC.add(p.get(CoreAnnotations.TopicAnnotation.class) + '-'
			// + cWord + "--CWORD");
			...
			// featuresC.add(p.get(CoreAnnotations.TopicAnnotation.class) + '-'
			// + c.get(CoreAnnotations.TopicAnnotation.class) + '-' +
			// n.get(CoreAnnotations.TopicAnnotation.class) + "-PCNTopicID");
			// featuresC.add(c.get(CoreAnnotations.TopicAnnotation.class) + '-'
			// + n.get(CoreAnnotations.TopicAnnotation.class) + "-CNTopicID");
			// featuresC.add(p.get(CoreAnnotations.TopicAnnotation.class) + '-'
			// + c.get(CoreAnnotations.TopicAnnotation.class) + "-PCTopicID");
			// featuresC.add(c.get(CoreAnnotations.TopicAnnotation.class) +
			// cShape + "-TopicID-SH");
			// asdasd
		}

		// todo [cdm 2014]: Have this guarded by a flag and things would be a
		// little faster. Set flag in current uses of this annotation.
		...
		if (c.get(CoreAnnotations.StackedNamedEntityTagAnnotation.class) != null) {
			...
			featuresC.add(
					cWord + "-" + c.get(CoreAnnotations.StackedNamedEntityTagAnnotation.class) + "-WCStackedNERTag");
		...
		if (flags.useRadical && cWord.length() > 0) {
			// todo [cdm 2016]: Really all stuff in this file should be fixed to
			// work with codepoints outside BMP
			if (cWord.length() == 1) {
		...
		if (flags.useInternal && flags.useExternal) {
			...
			if (flags.usePrev) {
				if (flags.useSequences && flags.usePrevSequences) {
					...
					if (!flags.strictGoodCoNLL) {
						featuresCpC.add(pWord + '-' + cWord + "-PSEQW2"); // added
																			// later
																			// after
																			// goodCoNLL
						featuresCpC.add(pWord + "-PSEQpW"); // added later after
															// goodCoNLL
					}
			...
			if (((flags.wordShape > WordShapeClassifier.NOWORDSHAPE) || flags.useShapeStrings) && flags.useTypeSeqs
				...
				if (flags.useTypeSeqs3) {
					featuresCpC.add(
							pShape + '-' + cShape + '-' + n.get(CoreAnnotations.ShapeAnnotation.class) + "-PCNSHAPES");
				}
				if (flags.useTypeSeqs2) {
					featuresCpC.add(pShape + '-' + cShape + "-TYPES"); // this
																		// duplicates
																		// PSEQpcS
																		// above
				}
			...
			if (flags.useTypeySequences) {
				...
				// featuresCpC.add(pShape) + "-" + cShape) + "-TPS"); //
				// duplicates -TYPES, so now omitted; you may need to slightly
				// increase sigma to duplicate previous results, however.
			}
	...
	protected Collection<String> featuresCp2C(PaddedList<IN> cInfo, int loc) {
		...
		if (flags.useMoreAbbr) {
			featuresCp2C.add(p2.get(CoreAnnotations.AbbrAnnotation.class) + '-'
					+ c.get(CoreAnnotations.AbbrAnnotation.class) + "-P2ABBRANS");
		}

		if (flags.useMinimalAbbr) {
			featuresCp2C.add(p2.get(CoreAnnotations.AbbrAnnotation.class) + '-'
					+ c.get(CoreAnnotations.AbbrAnnotation.class) + "-P2AP2CABB");
		}
		...
		if (flags.useParenMatching) {
			if (flags.useReverse) {
				if (cWord.equals("(") || cWord.equals("[") || cWord.equals("-LRB-")) {
					if ((p2Word.equals(")") || p2Word.equals("]") || p2Word.equals("-RRB-"))
				...
				if (cWord.equals(")") || cWord.equals("]") || cWord.equals("-RRB-")) {
					if ((p2Word.equals("(") || p2Word.equals("[") || p2Word.equals("-LRB-"))
							&& !(pWord.equals("(") || pWord.equals("[") || pWord.equals("-LRB-"))) {
						featuresCp2C.add("PAREN-MATCH");
					}
				}
			}
		...
		if (flags.useParenMatching) {
			if (flags.useReverse) {
				if (cWord.equals("(") || cWord.equals("[")) {
					if ((flags.maxLeft >= 3) && (p3Word.equals(")") || p3Word.equals("]"))
							&& !(p2Word.equals(")") || p2Word.equals("]") || pWord.equals(")") || pWord.equals("]"))) {
	...
	protected Collection<String> featuresCp4C(PaddedList<IN> cInfo, int loc) {
		...
		if (flags.useParenMatching) {
			if (flags.useReverse) {
				if (cWord.equals("(") || cWord.equals("[")) {
					if ((flags.maxLeft >= 4) && (p4Word.equals(")") || p4Word.equals("]"))
							&& !(p3Word.equals(")") || p3Word.equals("]") || p2Word.equals(")") || p2Word.equals("]")
									|| pWord.equals(")") || pWord.equals("]"))) {
	...
	protected Collection<String> featuresCp5C(PaddedList<IN> cInfo, int loc) {
		...
		if (flags.useParenMatching) {
			if (flags.useReverse) {
				if (cWord.equals("(") || cWord.equals("[")) {
					if ((flags.maxLeft >= 5) && (p5Word.equals(")") || p5Word.equals("]"))
							&& !(p4Word.equals(")") || p4Word.equals("]") || p3Word.equals(")") || p3Word.equals("]")
									|| p2Word.equals(")") || p2Word.equals("]") || pWord.equals(")")
									|| pWord.equals("]"))) {
	...
	protected Collection<String> featuresCpCp2C(PaddedList<IN> cInfo, int loc) {
		...
		if (flags.useInternal && flags.useExternal) {
			...
			if (flags.strictGoodCoNLL && !flags.removeStrictGoodCoNLLDuplicates && flags.useTypeySequences
					&& flags.maxLeft >= 2) {
				// this feature duplicates -TYPETYPES below, so probably don't
				// include it, but it was in original tests of CMM goodCoNLL
				featuresCpCp2C.add(p2.get(CoreAnnotations.ShapeAnnotation.class) + '-'
						+ p.get(CoreAnnotations.ShapeAnnotation.class) + '-'
						+ c.get(CoreAnnotations.ShapeAnnotation.class) + "-TTPS");
			}

			if (flags.useAbbr) {
				featuresCpCp2C.add(
						p2.get(CoreAnnotations.AbbrAnnotation.class) + '-' + p.get(CoreAnnotations.AbbrAnnotation.class)
	...
	private Collection<String> occurrencePatterns(PaddedList<IN> cInfo, int loc) {
		...
		if (cInfo.get(loc - reverse(1)).getString(CoreAnnotations.PartOfSpeechAnnotation.class) != null
				&& isNameCase(pWord)
				&& cInfo.get(loc - reverse(1)).getString(CoreAnnotations.PartOfSpeechAnnotation.class).equals("NNP")) {
			...
			for (int jump = 3; jump < 150; jump++) {
				if (getWord(cInfo.get(loc + reverse(jump))).equals(word)) {
					if (isNameCase(getWord(cInfo.get(loc + reverse(jump - 1)))) && (cInfo.get(loc + reverse(jump - 1)))
							.getString(CoreAnnotations.PartOfSpeechAnnotation.class).equals("NNP")) {
		...
		/*
		 * if (!l.isEmpty()) { log.info(pWord+" "+word+" "+nWord+" "+l); }
		 */
		return l;
	}
