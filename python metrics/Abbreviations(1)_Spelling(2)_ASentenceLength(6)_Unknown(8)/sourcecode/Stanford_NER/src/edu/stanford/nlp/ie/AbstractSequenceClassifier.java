...
public abstract class AbstractSequenceClassifier<IN extends CoreMap> implements Function<String, String> {
	...
	/**
	 * Different threads can add or query knownLCWords at the same time, so we need
	 * a concurrent data structure. Created in reinit().
	 */
	...
	/**
	 * This is the default DocumentReaderAndWriter used for reading text files for
	 * runtime classification. It is the DocumentReaderAndWriter specified by the
	 * plainTextDocumentReaderAndWriter flag and defaults to
	 * {@code edu.stanford.nlp.sequences.PlainTextDocumentReaderAndWriter} which is
	 * suitable for reading plain text files, in languages with a Tokenizer
	 * available.
	 *
	...
	/**
	 * Construct a SeqClassifierFlags object based on the passed in properties, and
	...
	public AbstractSequenceClassifier(SeqClassifierFlags flags) {
		...
		if (flags.featureFactory != null) {
			FeatureFactory<IN> factory = new MetaClass(flags.featureFactory).createInstance(flags.featureFactoryArgs); // for
																														// compatibility
	 ...
	 * the file, but we should probably stop serializing them and just reinitialize
	 * them from the flags?
	 */
	protected final void reinit() {
		...
		if (knownLCWords == null || knownLCWords.isEmpty()) {
			// reinit limits max (additional) size. We temporarily loosen this during
			// training
	 ...
	 * Useful for reading in untokenized text documents or reading plain text from
	 * the command line. An example of a way to use this would be to return a
	 * edu.stanford.nlp.wordseg.Sighan2005DocumentReaderAndWriter for the Chinese
	 * Segmenter.
	 */
	...
	/**
	 * Classify a List of IN. This method returns a new list of tokens, not the list
	 * of tokens passed in, and runs the new tokens through ObjectBankWrapper. (Both
	 * these behaviors are different from that of the classify(List) method.
	 *
	 * @param tokenSequence
	 *            The List of IN to be classified.
	 * @return The classified List of IN, where the classifier output for each token
	 *         is stored in its
	 *         {@link edu.stanford.nlp.ling.CoreAnnotations.AnswerAnnotation} field.
	 */
	...
	private List<IN> preprocessTokens(List<? extends HasWord> tokenSequence) {
		// log.info("knownLCWords.size is " + knownLCWords.size() + ";
		// knownLCWords.maxSize is " + knownLCWords.getMaxSize() +
		...
		// log.info("Size of knownLCWords is " + knownLCWords.size() + ", after NER for
		// " + getClass().toString());
		return document;
	}
	 ...
	 * globalInfo. Used by SUTime (NumberSequenceClassifier), which requires the doc
	 * date to resolve relative dates.
	 *
	 * @param tokenSequence
	 *            The List of IN to be classified.
	 * @return The classified List of IN, where the classifier output for each token
	 *         is stored in its "answer" field.
	 */
	...
	/**
	 * Takes a list of tokens and provides the K best sequence labelings of these
	 * tokens with their scores.
	 *
	 * @param doc
	 *            The List of tokens
	 * @param answerField
	 *            The key for each token into which the label for the token will be
	 *            written
	 * @param k
	 *            The number of best sequence labelings to generate
	 * @return A Counter where each key is a List of tokens with labels written in
	 *         the answerField and its value is the score (conditional probability)
	 *         assigned to this labeling of the sequence.
	 */
	...
	private DFSA<String, Integer> getViterbiSearchGraph(List<IN> doc,
	 ...
	 * and the {@link PlainTextDocumentReaderAndWriter} is used. The classifier will
	 * tokenize the text and treat each sentence as a separate document. The output
	 * can be specified to be in a choice of three formats: slashTags (e.g.,
	 * Bill/PERSON Smith/PERSON died/O ./O), inlineXML (e.g., &lt;PERSON&gt;Bill
	 * Smith&lt;/PERSON&gt; went to &lt;LOCATION&gt;Paris&lt;/LOCATION&gt; .), or
	 * xml, for stand-off XML (e.g., &lt;wi num="0"
	 * entity="PERSON"&gt;Sue&lt;/wi&gt; &lt;wi num="1"
	 ...
	 * <p>
	 * <i>Fine points:</i> The slashTags and xml formats show tokens as transformed
	 * by any normalization processes inside the tokenizer, while inlineXML shows
	 * the tokens exactly as they appeared in the source text. When a period counts
	 * as both part of an abbreviation and as an end of sentence marker, it is
	 * included twice in the output String for slashTags or xml, but only once for
	 * inlineXML, where it is not counted as part of the abbreviation (or any named
	 * entity it is part of). For slashTags with preserveSpacing=true, there will be
	 * two successive periods such as "Jr.." The tokenized (preserveSpacing=false)
	 * output will have a space or a newline after the last token.
	 *
	 * @param sentences
	...
	/**
	 * Classify the contents of a {@link String}. Plain text or XML is expected and
	 * the {@link PlainTextDocumentReaderAndWriter} is used by default. The
	 * classifier will treat each sentence as a separate document. The output can be
	 * specified to be in a choice of formats: Output is in inline XML format (e.g.
	 ...
	 * or XML input is expected and the {@link PlainTextDocumentReaderAndWriter} is
	 * used by default. Output looks like: My/O name/O is/O Bill/PERSON Smith/PERSON
	 * ./O
	 *
	 * @param sentences
	 *            The String to be classified
	 ...
	 * original String. Character offsets can be thought of as fenceposts between
	 * the characters, or, like certain methods in the Java String class, as
	 * character positions, numbered starting from 0, with the end index pointing to
	 * the position AFTER the entity ends. That is, end - start is the length of the
	 ...
	 * marker, and that abbreviation is part of a named entity, the reported entity
	 * string excludes the period.
	 *
	 * @param sentences
	 *            The string to be classified
	 * @return A {@link List} of {@link Triple}s, each of which gives an entity type
	 *         and the beginning and ending character offsets.
	 */
	...
	/**
	 * Have a word segmenter segment a String into a list of words. ONLY USE IF YOU
	 * LOADED A CHINESE WORD SEGMENTER!!!!!
	 *
	 * @param sentence
	 *            The string to be classified
	 * @return List of words
	 */
	// todo: This method is currently [2016] only called in a very small number of
	// places:
	// the parser's jsp webapp, ChineseSegmenterAnnotator, and SegDemo.
	// Maybe we could eliminate it?
	// It also seems like it should be using the plainTextReaderAndWriter, not
	// default?
	 ...
	 * scf.documentReader}.
	 *
	 * @return A {@link List} of {@link List}s of classified something that extends
	 * {@link CoreMap} where each {@link List} refers to a document/sentence.
	 */
	 ...
	 *         {@link edu.stanford.nlp.ling.CoreAnnotations.AnswerAnnotation} key).
	 *         The answers will be the class labels defined by the CRF Classifier.
	 *         They might be things like entity labels (in BIO notation or not) or
	 *         something like "1" vs. "0" on whether to begin a new token here or
	 *         not (in word segmentation).
	 */
	public abstract List<IN> classify(List<IN> document);

	/**
	 * Classify a {@link List} of something that extends {@link CoreMap} using as
	 * additional information whatever is stored in the document and sentence. This
	 * is needed for SUTime (NumberSequenceClassifier), which requires the document
	 * date to resolve relative dates.
	 *
	 * @param tokenSequence
	 * @param document
	...
	/**
	 * Classification is finished for the document. Do any cleanup (if information
	 * was stored as part of the document for global classification)
	 * 
	 * @param document
	 */
	...
	/**
	 * Train the classifier based on values in flags. It will use the first of these
	 * variables that is defined: trainFiles (and baseTrainDir), trainFileList,
	 * trainFile.
	 */
	public void train() {
	...
	public ObjectBank<List<IN>> makeObjectBankFromString(String string, DocumentReaderAndWriter<IN> readerAndWriter) {
		...
		// TODO
		return new ObjectBankWrapper<>(flags,
				new ObjectBank<>(new ResettableReaderIteratorFactory(string), readerAndWriter), knownLCWords);
	}
	...
	public ObjectBank<List<IN>> makeObjectBankFromFiles(String[] trainFileList,
		...
		// System.err.printf("trainFileList contains %d file%s in encoding %s.%n",
		// files.size(), files.size() == 1 ? "": "s", flags.inputEncoding);
		// TODO get rid of ObjectBankWrapper
		// return new ObjectBank<List<IN>>(new
		// ResettableReaderIteratorFactory(files), readerAndWriter);
		return new ObjectBankWrapper<>(flags,
				new ObjectBank<>(new ResettableReaderIteratorFactory(files, flags.inputEncoding), readerAndWriter),
				knownLCWords);
		// } catch (IOException e) {
	 ...
	 * reading method specified in {@code flags.documentReader}, and for some reader
	 * choices, the column mapping given in {@code flags.map}.
	 *
	 * @param in
	...
	public ObjectBank<List<IN>> makeObjectBankFromReader(BufferedReader in,
		...
		// readerAndWriter);
		return new ObjectBankWrapper<>(flags,
				new ObjectBank<>(new ResettableReaderIteratorFactory(in), readerAndWriter), knownLCWords);
	}
	...
	/**
	 * Takes a {@link List} of documents and prints the likelihood of each possible
	 * label at each point.
	 *
	 * @param documents
	...
	private static void outputCalibrationInfo(PrintWriter pw, Counter<Integer> calibration,
			Counter<Integer> correctByBin, TwoDimensionalCounter<Integer, String> calibratedTokens) {
		...
		for (int i = 0; i < numBins; i++) {
			pw.printf("[%.1f-%.1f%c: %.0f  %.2f%n", ((double) i) / numBins, ((double) (i + 1)) / numBins,
					i == (numBins - 1) ? ']' : ')', calibration.getCount(i),
					correctByBin.getCount(i) / calibration.getCount(i));
		}
		pw.println("----------------------------------------");
		for (int i = 0; i < numBins; i++) {
			pw.printf("[%.1f-%.1f%c: %s%n", ((double) i) / numBins, ((double) (i + 1)) / numBins,
					i == (numBins - 1) ? ']' : ')',
	...
	public Triple<Counter<Integer>, Counter<Integer>, TwoDimensionalCounter<Integer, String>> printProbsDocument(
			List<IN> document) {
	 ...
	 * flags.plainTextDocumentReaderAndWriter to determine how to read the textFile
	 * format. By default this gives
	 * edu.stanford.nlp.sequences.PlainTextDocumentReaderAndWriter. <i>Note:</i>
	 * This means that it works right for a plain textFile (and not a tab-separated
	 * columns test file).
	 *
	 * @param textFile
	 *            The file to test on.
	...
	public Triple<Double, Double, Double> classifyAndWriteAnswers(String testFile,
			DocumentReaderAndWriter<IN> readerWriter, boolean outputScores) throws IOException {
	...
	/**
	 * If the flag {@code outputEncoding} is defined, the output is written in that
	 ...
	 */
	public Triple<Double, Double, Double> classifyAndWriteAnswers(String testFile, OutputStream outStream,
			DocumentReaderAndWriter<IN> readerWriter, boolean outputScores, boolean textString) throws IOException {
		ObjectBank<List<IN>> documents;
		if (textString) {
			documents = makeObjectBankFromString(testFile, readerWriter);
		} else {
			documents = makeObjectBankFromFile(testFile, readerWriter);
		}
		PrintWriter pw = IOUtils.encodedOutputStreamPrintWriter(outStream, flags.outputEncoding, true);
		return classifyAndWriteAnswers(documents, pw, readerWriter, outputScores);
	}
	...
	public void classifyFilesAndWriteAnswers(Collection<File> testFiles, DocumentReaderAndWriter<IN> readerWriter,
			boolean outputScores) throws IOException {
		ObjectBank<List<IN>> documents = makeObjectBankFromFiles(testFiles, readerWriter);
	 ...
	 * @param readerWriter
	 * @param outputScores
	 *            Whether to calculate and output the performance scores (P/R/F1) of
	 *            the classifier
	 * @return A Triple of overall P/R/F1, if outputScores is true, else
	 *         {@code null}. The scores are done on a 0-100 scale like percentages.
	 * @throws IOException
	 */
	public Triple<Double, Double, Double> classifyAndWriteAnswers(Collection<List<IN>> documents,
			PrintWriter printWriter, DocumentReaderAndWriter<IN> readerWriter, boolean outputScores)
			throws IOException {
		...
		log.info(StringUtils.getShortClassName(this) + " tagged " + numWords + " words in " + numDocs + " documents at "
	...
	/**
	 * Run the classifier on the documents in an ObjectBank, and print the answers
	 * to a given PrintWriter (with timing to stderr). The value of
	...
	public void classifyAndWriteAnswersKBest(ObjectBank<List<IN>> documents, int k, PrintWriter printWriter,
		...
		for (List<IN> doc : documents) {
			...
			for (List<IN> l : sorted) {
				printWriter.println("<sentence id=" + numSentences + " k=" + n + " logProb=" + kBest.getCount(l)
		...
		double wordspersec = numWords / (((double) millis) / 1000);
		NumberFormat nf = new DecimalFormat("0.00"); // easier way!
		log.info(this.getClass().getName() + " tagged " + numWords + " words in " + numSentences + " documents at "
				+ nf.format(wordspersec) + " words per second.");
	}
	...
	public void classifyAndWriteViterbiSearchGraph(String testFile, String searchGraphPrefix,
		...
		for (List<IN> doc : documents) {
			...
			numWords += doc.size();
			PrintWriter latticeWriter = new PrintWriter(
					new FileOutputStream(searchGraphPrefix + '.' + numSentences + ".wlattice"));
			PrintWriter vsgWriter = new PrintWriter(
					new FileOutputStream(searchGraphPrefix + '.' + numSentences + ".lattice"));
	...
	/**
	 * Write the classifications of the Sequence classifier to a writer in a format
	...
	public static boolean countResultsSegmenter(List<? extends CoreMap> doc, Counter<String> entityTP,
	...
	private static Triple<Double, Double, Double> printPRLine(String entity, double tp, double fp, double fn) {
		...
		log.info(
				String.format("%15s\t%.4f\t%.4f\t%.4f\t%.0f\t%.0f\t%.0f%n", entity, precision, recall, f1, tp, fp, fn));
	...
	/**
	 * Loads a classifier from the given input stream. Any exceptions are rethrown
	 * as unchecked exceptions. This method does not close the InputStream.
	 *
	 * @param in
	...
	/**
	 * This function will load a classifier that is stored inside a jar file (if it
	 * is so stored). The classifier should be specified as its full path in a jar.
	 * If the classifier is not stored in the jar file or this is not run from
	 * inside a jar file, then this function will throw a RuntimeException.
	 *
	 * @param modelName
	 ...
	 */
	// todo [john bauer 2015]: This method may not be necessary. Perhaps use the
	// IOUtils equivalents
	...
	// Separating this method out lets printFeatureLists be inlined, which is good
	// since it is usually a no-op.
